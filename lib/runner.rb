...
rum_junctions_hq_bed = ARGV[3] # ../data/zfish_data/junctions_high-quality.bed
peptides_bed = ARGV[4] # peptides in bed format for ucsc # ../data/zfish_output/peptides.bed
...
danRer7_refseq_fap = FastaParser.new(danRer7_refseq)
unannotated_mrnas_seqs_outp = File.open(unannotated_mrnas_seqs, "w")
unannotated_mrnas_seqs_fap = FastaParser.open(unannotated_mrnas_seqs)
...
# 				puts "Creating alternative mRNA that could explain the junction"
				...
				if alt_mrna_checklist.has_key?(alternative_exon_starts.join(",") + "\t" + alternative_exon_stops.join(","))
					...
					puts "Found an alternative mrna !!!"
...
# call_check = system("bedtools getfasta -fi #{danRer7_refseq} -bed #{unannotated_mrnas_bed} -fo #{unannotated_mrnas_seqs} -s -name")
# if call_check == false
# 	puts "ERROR: #{unannotated_mrnas_seqs} not created"
# 	exit
# end
...
unannotated_mrnas_seqs_fap.each do |entry|
	...
	unannotated_proteins_outp.puts protein.to_fasta(entry.header.to_s, 60) #">" + entry.header.to_s + "\n" + protein.to_s + "\n"
end
